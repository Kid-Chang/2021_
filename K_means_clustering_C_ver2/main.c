...
int main()
{
    ...
    int randomN;
    for (int N = 0; N < k; N++)
    { //centroid에 임의의 한 data의 x, y 값 제공.
        // printf("%d번째 \n", N);
    ...
    for (int dataCount = 0; dataCount < n; dataCount++)
    {
        ...
        for (int clusterCount = 0; clusterCount < k; clusterCount++)
        {
            ...
            if (nowDistance < lowDistance)
            { // now가 low 보다 작다면 now값이 low로.
    ...
    while (flag < k)
    {
                ...
                for (int dataCount = 0; dataCount < n; dataCount++)
                {
                    ...
                    for (int clusterCount = 0; clusterCount < k; clusterCount++)
                    {
                        ...
                        { // now가 low 보다 작다면 now값이 low로.
                            ...
                            lowDistance = nowDistance;
                            data[dataCount].nowCluster = centroid[clusterCount].cluster; //k d n
                            yesPrintf = 1;
                        }
                    }
                    // printf("%d 번째 데이터의 클러스터는 %d\n \n", dataCount, data[dataCount].nowCluster);
        ...
        if (yesPrintf == 1)
        {
            ...
            int ClusterGetsu;
            for (ClusterGetsu = 0; ClusterGetsu < k; ClusterGetsu++)
            {
                int mutgae = 0; // 몇개?
                ...
                ketsuKey[iterationCount][ClusterGetsu] = mutgae;
                printf("total: %d and ketsuKey: %d \n", mutgae, ketsuKey[iterationCount][ClusterGetsu]);
            }
            canPrint = 0;
            if (ClusterGetsu > 1)
            {
                for (int ClusterGetsu = 1; ClusterGetsu < k; ClusterGetsu++)
                {
                    // printf("it's work\n");
                    printf("ketsuKey: %d %d\n", ketsuKey[iterationCount - 1][ClusterGetsu], ketsuKey[iterationCount][ClusterGetsu]);

                    if (ketsuKey[iterationCount - 1][ClusterGetsu] == ketsuKey[iterationCount][ClusterGetsu])
                    {
                        canPrint++;
                    }
                }
