...
int main()
{
    ...
    int totalNumOfNM[count_input + 1]; //행과 열 정보 받기. 행렬의 갯수+1 만큼 배열을 만들어서 m과 n값 일부를 담음
    ...
    { //입력확인용 코드.
        ...
        totalNumOfNM[j + 1] = martix_count[j][1]; //일렬로 배열.
    }
    ...
    for (int depth = 0; depth < count_input; depth++)
    {                                                              // 배열 자체가 몇 개인지.
    ...
    for (i = 0; i < count_input; i++)
    {
        for (j = 0; j < count_input - i; j++)
        {
            a = j;
            b = j + i;
            printf("지금 행렬 위치 : i: %d j: %d 좌표: %d %d \n", i, j, a, b);
            if (a == b)
            {
                calculate[a][b] = 0;
            }
    ...
    for (count = 0; count < count_input - 1; count++)
    { // count_input-1 만큼의 곱셈 계산 수행.
        for (a = 0; a < totalNumOfNM[count]; a++)
        { // 행
            for (b = 0; b < totalNumOfNM[count + 2]; b++)
            { // 열
                sum = 0;

                for (int k = 0; k < totalNumOfNM[count + 1]; k++)
                { // 내부 사이의 계산.
